...
final class AXEventHandler: CGSEventDelegate {
    ...
    func handleWindowMiniaturized(pid: pid_t, windowId: Int) {
        ...
        if let wsId, let controller {
            let nextToken = controller.workspaceManager.tiledEntries(in: wsId)
                .first { $0.token != token }?.token
            if let nextToken {
    ...
    private func replayManagedReplacementEvents(_ events: [PendingManagedReplacementEvent]) {
        for event in events.sorted(by: { $0.sequence < $1.sequence }) {
            ...
            case let .destroy(destroy):
                ...
                if windowExistsInWindowServer(windowId) {
                    WMLog.ax.info(
                        "Deferred destroy suppressed (still in window server): windowId=\(windowId)"
                    )
                    continue
                }
