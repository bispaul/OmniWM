    ...
    private func handleDisplayChange() {
        ...
        for monitorId in disconnectedIds {
            if let prev = previousMonitors[monitorId] {
                WMLog.workspace
                    .info("DisplayObserver: disconnected monitorId=\(String(describing: monitorId), privacy: .public)")
        ...
        for monitorId in connectedIds {
            if let monitor = currentById[monitorId] {
                WMLog.workspace
                    .info(
                        "DisplayObserver: connected monitorId=\(String(describing: monitorId), privacy: .public) frame=\(String(describing: monitor.frame), privacy: .public)"
                    )
        ...
        for monitorId in existingIds {
            ...
            if current.frame != previous.frame || current.visibleFrame != previous.visibleFrame {
                WMLog.workspace
                    .info(
                        "DisplayObserver: reconfigured monitorId=\(String(describing: monitorId), privacy: .public) frame=\(String(describing: current.frame), privacy: .public)"
                    )
