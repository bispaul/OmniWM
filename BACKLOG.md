...
|---|-------|--------|--------|-------------|----------|
| 23 | Sleep/wake window migration + frame write storm | BUG#2 | partial | Windows migrate to Retina on wake; displays disconnect/reconnect asynchronously (160ms gap); windows get wrong-monitor proportions | Delay rescan until all expected monitors reconnect, or preserve column widths across re-admission. See also REV: topology change debouncing (#52) |
