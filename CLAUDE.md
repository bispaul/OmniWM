...
Branch: `fix/scope-relayout-to-workspace`. Build 119. PID check: `pgrep -x OmniWM`.
...
| 35 | Viewport drift from ESV offset recomputation | **FIXED** | 119 | Two-source fix: (1) resolveSelection ESV guard — skip when same column, no structural change. (2) clampViewportOffset bounds made activeColumnIndex-aware. Zero drift verified via IPC (3 round-trips) + 203/203 tests pass. Memorygraph `fb2c982b`. |
| 34 | 32" display 1275px left gap — viewport at middle column | **FIXED** | 119 | Same root cause as Bug #35 — fixed by position-aware clamp bounds. |
| 36 | Workspace switch blinks (verificationMismatch) | **OPEN** | 119 | Fork-specific — not in upstream. 104 mismatches in 2min during WS switch. Y off by 4px on 32" display. Cause: fork triggers more frame writes per WS switch than upstream (multiple ESV, pipeline). Option B (read-only computeLayoutPlan) would reduce. Memorygraph `5afef9c1`. |
